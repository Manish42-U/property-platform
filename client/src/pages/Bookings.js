...
import { 
  ...
  FaHome, FaInfoCircle, FaEnvelope, FaPhone, 
  FaStar, FaStarHalf, FaRegStar, FaBed, FaBath,
  FaUser, FaBuilding, FaBell, FaCheck, FaTimes
} from 'react-icons/fa';
import { MdVerified, MdLocationOn, MdDateRange } from 'react-icons/md';
import { GiClockwork } from 'react-icons/gi';
  ...
  const fetchBookings = async () => {
    try {
      ...
      if (isOwner) {
        ...
        endpoint = 'http://localhost:3000/api/bookings/owner-bookings';
      } else {
        // User fetches their own bookings
        endpoint = 'http://localhost:3000/api/bookings/my-bookings';
      }
        ...
        <>
          <StatsGrid>
            <StatCard onClick={() => setActiveFilter('all')}>
              ...
              <div className="stat-number">{stats.total}</div>
              <div className="stat-label">Total {isOwner ? 'Requests' : 'Bookings'}</div>
            </StatCard>
          ...
          <FilterTabs>
            ...
            <FilterTab onClick={() => setActiveFilter('completed')} active={activeFilter === 'completed'}>
              Completed
            </FilterTab>
          ...
          {filteredBookings.map(booking => (
            <BookingCard key={booking._id}>
              <CardHeader>
                <div className="booking-type">
                  {booking.type === 'visit' ? <FaClock /> : <FaHome />}
                  <span>{booking.type === 'visit' ? 'Site Visit' : 'Rental'} {isOwner ? 'Request' : 'Booking'}</span>
                </div>
                <div className="booking-date">
                  <MdDateRange />
                  <span>{isOwner ? 'Requested' : 'Booked'} on: {formatDate(booking.createdAt)}</span>
                </div>
                <StatusBadge status={booking.status}>
                  {getStatusIcon(booking.status)}
                  {booking.status.toUpperCase()}
              ...
              <CardBody>
                  ...
                  <div>
                    ...
                    {isOwner ? (
                      ...
                      <UserSection>
                        ...
                        <div className="user-detail">
                          <FaUser />
                          <strong>{booking.property?.owner?.name || 'Property Owner'}</strong>
                        </div>
                        <div className="user-detail">
                          <FaEnvelope />
                          <a href={`mailto:${booking.property?.owner?.email}`} style={{ color: '#475569', textDecoration: 'none' }}>
                            {booking.property?.owner?.email || 'Not available'}
                          </a>
                        </div>
                        <div className="user-detail">
                          <FaPhone />
                          <a href={`tel:${booking.property?.owner?.phone}`} style={{ color: '#475569', textDecoration: 'none' }}>
                    ...
                    {booking.message && (
                      <MessageBox>
                        <div className="message-label">
                          <FaEnvelope />
                          <span>{isOwner ? 'Customer Message' : 'Additional Message'}</span>
                        </div>
                        <div className="message-text">{booking.message}</div>
                      </MessageBox>
                    )}
                ...
                {booking.status === 'confirmed' && booking.type === 'visit' && (
                  <ActionButtons>
                    <button className="contact" onClick={() => window.location.href = `tel:${isOwner ? booking.user?.phone : booking.property?.owner?.phone}`}>
